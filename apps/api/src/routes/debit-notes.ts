/**
 * Debit notes session API (Phase 4 Finance).
 */
...
import { createDebitNote, getDebitNote, getDebitNoteLines } from '../lib/finance';
...
function mapFail(f: string): { status: number; code: string; msg: string } {
  const map: Record<string, { status: number; code: string; msg: string }> = {
    not_found: { status: 404, code: 'NOT_FOUND', msg: 'Debit note not found' },
    ...
    validation: { status: 400, code: 'VALIDATION', msg: 'Invalid debit note payload' },
  };
  return map[f] ?? { status: 400, code: 'VALIDATION', msg: 'Invalid debit note' };
}

export function createDebitNotesRouter(
  db: Kysely<Database>,
  requirePermission: (p: string) => RequestHandler,
): ExpressRouter {
  const router = Router();
  const view = requirePermission('debit_notes:view');
  const create = requirePermission('debit_notes:create');
  ...
  router.get('/:id', view, async (req, res, next) => {
    try {
      const workspaceId = (req as AuthenticatedRequest).workspace.id;
      const dn = await getDebitNote(db, { workspaceId, id: req.params.id! });
      if (!dn) return fail(res, 404, 'NOT_FOUND', 'Debit note not found');
      const lines = await getDebitNoteLines(db, { workspaceId, debitNoteId: dn.id });
      res.json({ data: { ...dn, lines }, error: null });
    } catch (e) {
      next(e);
    }
  });
  ...
  router.post('/', create, async (req, res, next) => {
    try {
      ...
      const result = await withUnitOfWork(db, async (trx) =>
        createDebitNote(trx, { workspaceId, createdBy: userId, ...body }),
      );
      if (!result.ok) {
        const m = mapFail(result.fail);
        return fail(res, m.status, m.code, m.msg);
      }
      res.status(201).json({ data: { ...result.debitNote, lines: result.lines }, error: null });
    } catch (e) {
      next(e);
    }
  });
